...
void draw() {
  ...
  if (pins != null && pins.length > 0) {
    ...
    float redVal = map(pins[0], 0, 1023, 0, 255);
    float greenVal = map(pins[1], 0, 1023, 0, 255);
    ...
    // A0: pins[0]
    float mappedVal0 = map(pins[0], 0, 1023, height, 0);
    line(0, mappedVal0, width/3, mappedVal0);
    
    // A1: pins[1]
    float mappedVal1 = map(pins[1], 0, 1023, height, 0);
